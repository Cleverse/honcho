...
"""Smoke test: wait for deriver queue to drain."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Honcho smoke test: queue")
    add_connection_args(parser)
    parser.add_argument(
        "--queue-timeout",
        type=int,
        default=90,
        help="Max seconds to wait for queue to empty",
    )
    ...
    if state is None or not state.workspace_id:
        print(
            f"ERROR: state file missing workspace_id: {args.state_file}. "
            "Run scripts/smoke/test_workspace.py first.",
            file=sys.stderr,
        )
        return 1
